...
export function AgentConsoleSidebar({
 ...
 <div
 className={cn(
"flex flex-col h-full w-[360px] border-l bg-bg-root dark:bg-bg-card border-border-subtle transition-all duration-300 ease-in-out shrink-0 overflow-hidden relative",
 className
 )}
 >
 {/* Console Header */}
 <div className="flex items-center justify-between px-5 py-4 border-b border-border-subtle bg-bg-card dark:bg-bg-root">
 <div className="flex items-center gap-2">
 <Cpu className="w-5 h-5 text-accent-primary dark:text-accent-secondary animate-pulse" />
 ...
 <button
 onClick={onClose}
 className="p-1.5 rounded-lg text-text-muted hover:bg-bg-hover transition-colors active:scale-95"
 title="Đóng console"
 >
 ...
 <div className="bg-bg-card dark:bg-bg-root p-4 rounded-xl border border-border-subtle shadow-sm space-y-3">
...
"px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider",
 ...
 <div className="flex items-center justify-between">
 <span className="text-sm font-medium text-text-body">Spawning Score (S)</span>
 <span className="text-lg font-black text-accent-primary dark:text-accent-secondary">{score.toFixed(3)}</span>
 </div>
 
 {/* Visual Gauge */}
 <div className="w-full bg-bg-section h-2 rounded-full overflow-hidden relative">
 <div 
 className={cn(
"h-full rounded-full transition-all duration-500",
 ...
 <div className="grid grid-cols-2 gap-2 pt-2 text-[11px] text-text-muted border-t border-border-subtle/60">
 <div className="flex justify-between">
 <span>Context Pres. (w_c):</span>
 <span className="font-semibold text-text-body">{(breakdown.c_ratio || 0).toFixed(2)}</span>
 </div>
 <div className="flex justify-between">
 <span>Intent Comp. (w_d):</span>
 <span className="font-semibold text-text-body">{(breakdown.d_intent || 0).toFixed(1)}</span>
 </div>
 <div className="flex justify-between">
 <span>Retrieval Vol. (w_r):</span>
 <span className="font-semibold text-text-body">{(breakdown.r_docs || 0).toFixed(1)}</span>
 </div>
 <div className="flex justify-between">
 <span>Verification (w_v):</span>
 <span className="font-semibold text-text-body">{(breakdown.v_need || 0).toFixed(1)}</span>
 </div>
 </div>
 )}
 </div>
 ...
 <span className="font-semibold text-text-subheading">{consolidation.raw_tokens} tokens</span>
 </div>
 <div className="flex justify-between text-xs text-text-muted">
 <span>Consolidated Context:</span>
 <span className="font-semibold text-accent-primary dark:text-accent-secondary">{consolidation.consolidated_tokens} tokens</span>
 </div>
 <div className="flex items-center justify-between text-xs pt-1 border-t border-border-subtle/60">
 <span className="text-text-muted">Compression Savings:</span>
...
"px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider",
 ...
 <div className="flex flex-col items-center bg-bg-root dark:bg-bg-card p-2 rounded-lg border border-border-subtle">
 <span className="text-[10px] text-text-muted">Factuality</span>
 <span className={cn(
"text-sm font-bold mt-0.5",
 ...
 <div className="flex flex-col items-center bg-bg-root dark:bg-bg-card p-2 rounded-lg border border-border-subtle">
 <span className="text-[10px] text-text-muted">Pedagogy</span>
 <span className={cn(
"text-sm font-bold mt-0.5",
 ...
 <div className="flex flex-col items-center bg-bg-root dark:bg-bg-card p-2 rounded-lg border border-border-subtle">
 <span className="text-[10px] text-text-muted">Format</span>
 <span className={cn(
"text-sm font-bold mt-0.5",
 ...
 <div className="text-xs text-text-muted bg-bg-root dark:bg-bg-card p-2.5 rounded-lg border border-border-subtle/80 leading-relaxed max-h-36 overflow-y-auto">
 <span className="font-bold text-text-body block mb-1">Critic Feedback:</span>
 {critique.critique_report}
 </div>
 )}
 </div>
 ...
 <div className="space-y-3">
 <span className="text-xs font-bold uppercase tracking-wider text-text-muted block px-1">
 Execution Timeline
 </span>

 {logs.length === 0 ? (
 <div className="text-center py-8 bg-bg-card dark:bg-bg-root rounded-xl border border-border-subtle text-xs text-text-muted">
...
"text-rose-500 bg-rose-50 dark:bg-rose-950/20";
 ...
 <div
 key={log.subagentId}
 className="bg-bg-card dark:bg-bg-root rounded-xl border border-border-subtle overflow-hidden shadow-sm transition-all duration-200"
 >
 {/* Header */}
 <div 
 onClick={() => toggleLog(log.subagentId)}
 className="flex items-center justify-between p-3 cursor-pointer hover:bg-bg-section dark:hover:bg-bg-root transition-colors select-none"
 >
 <div className="flex items-center gap-2">
 ...
 <div>
 <h4 className="text-xs font-bold text-text-subheading">
 {log.role}
 </h4>
 <p className="text-[10px] text-text-muted dark:text-text-muted max-w-[200px] truncate">
 {log.task}
 </p>
 </div>
 </div>
 ...
 {isExpanded ? <ChevronUp className="w-3.5 h-3.5 text-text-disabled" /> : <ChevronDown className="w-3.5 h-3.5 text-text-disabled" />}
 </div>
 </div>

 {/* Collapsible log output */}
 {isExpanded && (
 <div className="border-t border-border-subtle/80 bg-bg-root p-3 text-[11px] font-mono text-emerald-400 dark:text-emerald-500 space-y-2">
 ...
 {log.summary && (
 <div className="text-[10px] text-text-disabled border-t border-border-subtle pt-2 font-sans italic leading-relaxed">
 Result: {log.summary}
 </div>
 )}
 {log.error && (
 <div className="text-[10px] text-rose-400 border-t border-border-subtle pt-2 font-sans italic leading-relaxed">
 Error: {log.error}
 </div>
 )}
 </div>
