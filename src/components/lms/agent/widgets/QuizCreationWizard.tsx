...
export function QuizCreationWizard({ props }: QuizCreationWizardProps) {
 ...
 <h3 className="text-xl font-bold text-text-heading mb-2">Quiz đã sẵn sàng!</h3>
 ...
 <div className="bg-bg-card rounded-2xl border border-border-subtle p-5 shadow-sm">
 <div className="flex items-center gap-3 mb-6">
 <div className="w-10 h-10 bg-blue-100 dark:bg-blue-900/30 rounded-xl flex items-center justify-center">
 <BookOpen className="w-5 h-5 text-accent-primary dark:text-accent-secondary" />
 </div>
 <div>
 <h2 className="text-lg font-bold text-text-heading">Cấu hình Quiz Activity</h2>
 <p className="text-xs text-text-muted">Thiết lập các thông số cơ bản cho bài kiểm tra.</p>
 </div>
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
 <div className="space-y-1.5">
 <label className="text-xs font-semibold text-text-muted uppercase ml-1">Tên bài kiểm tra</label>
 <input
 type="text"
 value={title}
 onChange={(e) => setTitle(e.target.value)}
 placeholder="Nhập tiêu đề quiz..."
 className="w-full px-4 py-2.5 rounded-xl border border-border-input bg-bg-section focus:ring-2 focus:ring-blue-500 outline-none transition-all"
 />
 </div>

 <div className="space-y-1.5">
 <label className="text-xs font-semibold text-text-muted uppercase ml-1">Thời gian (phút)</label>
 <div className="relative">
 <input
 type="number"
 value={duration}
 onChange={(e) => setDuration(parseInt(e.target.value) || 0)}
 className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-border-input bg-bg-section focus:ring-2 focus:ring-blue-500 outline-none transition-all"
 />
 <Clock className="w-4 h-4 text-text-disabled absolute left-3.5 top-1/2 -translate-y-1/2" />
 </div>
 </div>

 <div className="md:col-span-2 space-y-1.5">
 <label className="text-xs font-semibold text-text-muted uppercase ml-1">Chọn Chương (Section)</label>
 <select
 ...
 <label className="text-xs font-semibold text-text-muted uppercase ml-1">Tên chương mới</label>
 <div className="relative">
 <input
 type="text"
 ...
 <span className="px-2 py-0.5 rounded-full bg-bg-section text-text-muted">
 ...
 className="text-xs font-medium text-accent-primary dark:text-accent-secondary hover:underline"
 >
 Duyệt tất cả
 </button>
 </div>
...
"rounded-2xl border p-4 transition-all duration-200 group",
 ...
 :"border-border-subtle bg-bg-card shadow-sm hover:shadow-md",
 )}
 >
...
"text-[10px] font-bold px-2 py-0.5 rounded-lg uppercase tracking-wider",
 ...
 <p className="text-sm font-medium text-text-subheading leading-relaxed">
 {d.question_text}
 </p>
 </div>
...
"w-9 h-9 rounded-xl flex items-center justify-center transition-all active:scale-90",
 ...
 :"bg-bg-section text-text-disabled hover:bg-green-100 hover:text-green-600 dark:hover:bg-green-900/30"
 )}
 >
...
"w-9 h-9 rounded-xl flex items-center justify-center transition-all active:scale-90",
 ...
 :"bg-bg-section text-text-disabled hover:bg-red-100 hover:text-red-500 dark:hover:bg-red-900/30"
 )}
 >
 <X className="w-5 h-5" />
 </button>
 ...
 className="mt-3 text-[11px] font-semibold text-text-disabled hover:text-blue-500 transition-colors flex items-center gap-1.5"
 >
 ...
 {isExpanded && (
 <div className="mt-4 space-y-2 border-t border-border-subtle pt-4 animate-in slide-in-from-top-1">
...
"flex items-start gap-3 px-4 py-2.5 rounded-xl text-sm transition-colors",
 ...
 :"bg-bg-section dark:bg-bg-hover text-text-muted"
 )}
 >
 ...
 {/* ── Footer ── */}
 <div className="pt-4 sticky bottom-0 bg-gradient-to-t from-bg-card via-bg-card to-transparent pb-2">
 {error && (
...
"w-full h-14 rounded-2xl font-bold text-white shadow-xl flex items-center justify-center gap-3 transition-all active:scale-[0.98]",
 isSaving 
 ?"bg-text-disabled dark:bg-bg-hover cursor-not-allowed" 
 :"bg-accent-primary hover:bg-accent-primary-hover shadow-blue-200 dark:shadow-none"
 )}
 >
 {isSaving ? (
 <>
