...
function QuizCard({
 ...
 <p className="font-semibold text-text-heading truncate">{quiz.title}</p>
 )}
 ...
 <div>
 <label className="block text-xs font-medium mb-1 text-text-body">Mô tả</label>
 <input
 value={summary}
 ...
 <div key={qIdx} className="rounded-lg border border-border-input p-3 space-y-2">
 <div className="flex items-center justify-between">
 <div className="flex items-center gap-2">
 <span className="text-xs font-bold text-text-muted">Câu {qIdx + 1}</span>
 ...
 className="w-full px-2 py-1.5 rounded border border-border-input dark:border-border-subtle bg-bg-card text-sm"
 ...
 : <Circle className="w-4 h-4 text-text-disabled dark:text-text-muted" />}
 </button>
 <input
 value={opt.text}
 ...
 :"border-border-input dark:border-border-subtle bg-bg-card"
 }`}
 ...
 className="text-xs text-accent-primary dark:text-accent-secondary hover:underline flex items-center gap-1 ml-6"
 >
 ...
 <div>
 <label className="block text-[10px] font-medium text-text-muted mb-0.5">Giải thích</label>
 <textarea
 rows={1}
 value={q.explanation}
 onChange={e => updateQuestion(qIdx,"explanation", e.target.value)}
 className="w-full px-2 py-1 rounded border border-border-input dark:border-border-subtle bg-bg-card text-xs"
 ...
 <p className="text-text-subheading mb-1">{q.question}</p>
 ...
 {q.explanation && (
 <p className="text-xs text-text-muted italic mt-1 ml-2">💡 {q.explanation}</p>
 )}
 </div>
 ))}
 </div>
