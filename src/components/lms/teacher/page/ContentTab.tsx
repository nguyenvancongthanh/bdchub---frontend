...
export function ContentTab({ courseId, sections, onSectionsChange }: ContentTabProps) {
 ...
 className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-semibold bg-bg-card border border-border-input hover:bg-bg-section dark:hover:bg-bg-hover text-text-body transition-colors"
 ...
 {/* ── Section header ── */}
 <div
 className="flex items-center gap-3 px-5 py-4 bg-bg-section dark:bg-bg-hover cursor-pointer hover:bg-bg-hover transition-colors"
 onClick={() => toggle(sec.id)}
 >
 {/* Index badge */}
 <div className="w-7 h-7 rounded-lg bg-blue-50 dark:bg-blue-950/20 text-accent-primary dark:text-accent-secondary flex items-center justify-center text-xs font-bold flex-shrink-0 border border-blue-200 dark:border-blue-800">
 {i + 1}
 </div>
 ...
 <button
 title="Thêm nội dung"
 className="p-1.5 rounded-lg hover:bg-bg-card dark:hover:bg-bg-hover text-text-muted transition-colors"
 ...
 {isExpanded
 ? <ChevronDown className="w-4 h-4 text-text-disabled flex-shrink-0" />
 ...
 {[0, 1, 2].map(k => (
 <div key={k} className="h-8 bg-bg-section rounded-lg animate-pulse" />
 ))}
 </div>
 ) : contents.length === 0 ? (
 ...
 <div
 key={c.id}
 className="flex items-center gap-3 px-5 py-3 group hover:bg-bg-hover/30 transition-colors"
 >
 {/* Index */}
 <span className="text-text-disabled flex-shrink-0 w-4 text-xs text-right">
 {ci + 1}
 </span>
 ...
 <p className="flex-1 text-sm font-medium text-text-subheading truncate">
 {c.title}
 </p>
 ...
 <div className="bg-bg-card rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
