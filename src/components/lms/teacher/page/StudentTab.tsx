...
export function StudentsTab({ courseId }: Props) {
 ...
 {[0,1,2,3].map(i => (
 <div key={i} className="h-20 bg-bg-section rounded-2xl" />
 ))}
 </div>
 <div className="h-10 bg-bg-section rounded-xl w-80" />
 <div className="h-64 bg-bg-section rounded-2xl" />
 </div>
 );
 }
 ...
 className="w-full pl-9 pr-4 py-2.5 border border-border-input rounded-xl bg-bg-card text-text-heading placeholder:text-text-disabled text-sm focus:outline-none focus:ring-2 focus:ring-border-focus/20 focus:border-border-focus transition-all"
 />
 </div>
 ...
 disabled={refreshing}
 className="p-2 rounded-lg text-text-muted hover:text-text-body dark:hover:text-text-disabled hover:bg-bg-hover transition-all disabled:opacity-40"
 title="Làm mới"
 >
...
"from-pink-400 to-pink-600",
 ...
 return (
 <div className="bg-bg-card rounded-2xl border border-border-subtle shadow-lg overflow-hidden">
 {/* Header */}
 <div className="flex items-center justify-between px-4 py-3 border-b border-border-subtle">
 <span className="text-sm font-bold text-text-subheading">Chi tiết học viên</span>
 <button
 onClick={onClose}
 className="p-1.5 rounded-lg text-text-disabled hover:text-text-muted hover:bg-bg-hover transition-colors"
 >
 <X className="w-4 h-4" />
 </button>
 </div>
 ...
 <p className="text-xs text-text-disabled flex items-center gap-1 mt-0.5">
 ...
 <span className="text-text-muted">Tiến độ bắt buộc</span>
 ...
 <div className="flex items-center justify-between p-3 bg-bg-section dark:bg-bg-hover rounded-xl">
 <div className="flex items-center gap-2">
 <Award className="w-4 h-4 text-violet-500" />
 <span className="text-xs text-text-muted">Điểm quiz TB</span>
 </div>
 ...
 className="block w-full text-center py-2 bg-accent-primary hover:bg-accent-primary-hover text-white text-sm font-semibold rounded-xl transition-all active:scale-95"
 >
 Liên hệ học viên
 </Link>
 </div>
