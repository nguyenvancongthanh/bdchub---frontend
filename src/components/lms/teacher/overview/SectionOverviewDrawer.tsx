...
export function SectionOverviewDrawer({
 ...
 <button
 onClick={onClose}
 className="p-1.5 rounded-lg hover:bg-bg-hover flex-shrink-0"
 >
 <ChevronLeft className="w-4 h-4" />
 </button>
 <BookOpen className="w-4 h-4 text-indigo-500 flex-shrink-0" />
 <h2 className="font-bold text-text-heading truncate">
...
function JobProgressBanner({ job }: { job: SectionOverviewJobDetail["job"] }) {
 return (
 <div className="px-6 py-4 border-b border-border-subtle bg-yellow-50 dark:bg-yellow-950/20">
...
function LessonTab({
 ...
 if (!lesson) {
 return (
 <div className="flex items-center justify-center py-12 text-text-disabled text-sm">
 ...
 className="w-full px-3 py-2 rounded-lg border border-border-input dark:border-border-subtle bg-bg-card text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-60"
 />
 </div>
 ...
 disabled={isPublished}
 className="w-full px-3 py-2 rounded-lg border border-border-input dark:border-border-subtle bg-bg-card text-sm font-mono resize-y focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-60"
 />
 </div>
 ...
 onClick={save}
 disabled={saving}
 className="px-4 py-2 rounded-lg text-sm font-semibold bg-bg-section hover:bg-bg-hover text-text-body inline-flex items-center gap-1.5 disabled:opacity-60 transition-all duration-200 active:scale-95"
 >
 ...
 >
 <div
 className="bg-bg-card rounded-2xl max-w-sm w-full shadow-xl p-6"
 ...
 <p className="text-xs text-text-disabled italic mt-3">
...
function QuizTab({
 ...
 <p className="text-sm font-semibold text-text-heading flex-1 truncate">
 {quiz.title}
 </p>
...
const BLOOM_COLORS: Record<string, string> = {
 remember:"bg-bg-section text-text-muted",
...
function QuestionCard({
 ...
 className="w-full px-2 py-1 rounded border border-border-input dark:border-border-subtle bg-bg-card text-sm"
 />
 ) : (
 ...
 <span
 key={id}
 className="px-1.5 py-0.5 rounded text-xs bg-bg-section text-text-muted"
 >
 #{id}
 </span>
 ))}
 ...
 :"bg-bg-section dark:bg-bg-hover border border-border-subtle"
 }`}
 >
 {isEditing ? (
 <>
 ...
 className="flex-1 bg-transparent border-none outline-none text-sm text-text-subheading"
 />
 </>
 ) : (
 <>
...
function AgentLogsAccordion({ job }: { job: SectionOverviewJobDetail["job"] }) {
 ...
 <details
 className="group border border-border-input rounded-xl bg-bg-root dark:bg-bg-card/50 overflow-hidden"
 open={open}
 onToggle={(e) => setOpen((e.target as HTMLDetailsElement).open)}
 >
 <summary className="flex items-center justify-between px-4 py-3 font-semibold text-xs text-text-body cursor-pointer hover:bg-bg-hover select-none">
 <div className="flex items-center gap-2">
 <Terminal className="w-3.5 h-3.5 text-indigo-500" />
 <span>Xem lịch sử suy nghĩ & làm việc của các AI Agents</span>
 </div>
 <span className="text-[10px] text-text-disabled group-open:rotate-180 transition-transform duration-200">
 ▼
 </span>
 </summary>
 <div className="border-t border-border-input bg-bg-root p-4 font-mono text-[11px] text-text-disabled dark:text-text-disabled overflow-x-auto max-h-60 overflow-y-auto space-y-1">
 {logLines.map((line, idx) => {
 let color ="text-text-disabled";
