...
export function QuizRenderer({
 ...
 if (loading) {
 return (
 <div className="flex items-center gap-3 p-6 bg-bg-card border border-border-subtle rounded-2xl">
 <div className="w-5 h-5 border-2 border-blue-600 border-t-transparent rounded-full animate-spin flex-shrink-0" />
 <span className="text-text-muted text-sm">Đang tải thông tin quiz...</span>
 </div>
 );
 }
 ...
 <p className="font-semibold text-text-heading mb-1">{content.title}</p>
 {content.description && (
 ...
 <div className="flex flex-col gap-0.5 mt-3 text-xs text-text-muted">
 {quiz.available_from && (
...
"flex-1 py-3.5 rounded-xl font-semibold text-sm transition-all active:scale-95 shadow-sm",
 ...
 className="px-4 py-3.5 bg-bg-card border border-border-input text-text-body hover:bg-bg-hover rounded-xl font-medium text-sm transition-all active:scale-95"
 >
 📜 Lịch sử
 </button>
 </div>
