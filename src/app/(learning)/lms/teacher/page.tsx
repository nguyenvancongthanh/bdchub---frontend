...
function ActionCard({
 ...
 const VARIANT = {
 default:"border-border-subtle hover:border-border-hover bg-bg-card",
 ...
 <div>
 <p className="font-semibold text-text-subheading text-sm">{title}</p>
 <p className="text-xs text-text-muted dark:text-text-muted mt-0.5">{description}</p>
 </div>
 </button>
 );
}
...
export default function TeacherDashboard() {
 ...
 <div>
 <p className="text-sm text-text-muted dark:text-text-muted uppercase tracking-wider font-semibold mb-1">Giảng viên</p>
 ...
 <h3 className="font-bold text-text-subheading text-sm mb-4 flex items-center gap-1.5">
...
"Điểm Quiz (%)": s.avgQuiz ? Math.round(s.avgQuiz) : 0
 ...
 <ActionCard icon={<BookOpen className="w-6 h-6 text-text-muted" />}
 ...
 <table className="min-w-full divide-y divide-border-subtle dark:divide-border-section text-sm">
 <thead>
 ...
 <tbody className="divide-y divide-border-section dark:divide-border-section/50">
 ...
 {/* Thumbnail */}
 <div className="w-14 h-9 rounded-lg overflow-hidden bg-bg-section border border-border-subtle flex-shrink-0 relative">
 {stat.thumbnail_url ? (
 <img src={stat.thumbnail_url} alt={stat.title} className="object-cover w-full h-full" />
 ) : (
 <div className="w-full h-full flex items-center justify-center bg-bg-section">
 <BookOpen className="w-4 h-4 text-text-disabled" />
 </div>
 )}
 </div>
 <span className="font-semibold text-text-heading line-clamp-1">{stat.title}</span>
 </div>
 </td>

 {/* Learners count */}
 <td className="py-4">
 <span className="font-semibold text-text-body">{stat.studentCount} học viên</span>
 </td>
 ...
 <td className="py-4">
 {stat.avgQuiz !== null ? (
 <span className="font-bold text-text-subheading">{Math.round(stat.avgQuiz)}%</span>
 ) : (
 <span className="text-text-disabled dark:text-text-muted">— (Không có Quiz)</span>
 )}
 </td>
 ...
 className="text-xs font-semibold py-1.5 px-3 rounded-lg bg-bg-section hover:bg-bg-hover text-text-body transition-colors"
 >
 Chi tiết
 </button>
 </td>
