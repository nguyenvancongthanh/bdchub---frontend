...
export default function StudentQuizTakingPage() {
 ...
 <div 
 className="relative max-w-3xl w-full bg-bg-card rounded-2xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden border border-border-subtle animate-in fade-in zoom-in-95 duration-200"
 onClick={(e) => e.stopPropagation()}
 >
 {/* Header */}
 <div className="p-6 border-b border-border-subtle bg-bg-root dark:bg-bg-card/50">
 ...
 <p className="text-text-muted font-medium">Đang tải đáp án từ máy chủ...</p>
 </div>
 ) : (
 ...
 :"bg-bg-section dark:bg-bg-hover border-border-subtle"
 }`}>
 <span className={`block text-2xl font-extrabold ${unansweredCount > 0 ?"text-amber-700 dark:text-amber-400" :"text-text-muted"}`}>{unansweredCount}</span>
 <span className="text-xs text-text-muted/80 dark:text-text-disabled/80 font-semibold uppercase tracking-wider">Chưa hoàn tất</span>
 </div>
 </div>

 <div className="divide-y divide-border-section dark:divide-border-section border border-border-subtle rounded-xl overflow-hidden bg-bg-section/50 dark:bg-bg-card/20">
 {questions.map((q, idx) => {
 ...
 <div 
 key={q.id} 
 className={`p-4 flex items-start justify-between gap-4 hover:bg-bg-hover/10 transition-colors ${
 q.is_required && !isAnswered ?"bg-red-500/5" :""
 }`}
 >
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2 mb-1">
 <span className="font-semibold text-text-subheading">
 Câu {idx + 1}
 </span>
 {q.is_required && (
 ...
 <div className="text-sm font-medium text-text-body bg-bg-card dark:bg-bg-hover border border-border-subtle rounded-lg px-3 py-2">
 {ansText}
 </div>
 </div>
 ...
 {/* Footer */}
 <div className="p-6 border-t border-border-subtle bg-bg-root dark:bg-bg-card/50 space-y-4">
 ...
 disabled={submitting}
 className="px-5 py-2.5 border-border-input text-text-body rounded-xl hover:bg-bg-hover/80 transition-all font-semibold"
 >
 Quay lại làm tiếp
 </Button>
 <Button
...
"Xác nhận nộp bài"
 ...
 <div className="relative rounded-lg overflow-hidden border-2 border-border-subtle hover:border-blue-400 dark:hover:border-blue-600 transition-all">
 <img
 src={image.url}
 ...
 <button
 onClick={() => setShowImageModal(image.url)}
 className="absolute top-2 right-2 bg-bg-card bg-opacity-90 hover:bg-opacity-100 rounded-full p-2 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"
 title="Xem ảnh lớn"
 >
 🔍
 </button>
 ...
 return (
 <div className="bg-bg-card rounded-xl shadow-lg border-2 border-border-subtle p-8">
 {/* Question Header */}
 ...
 * Bắt buộc
 ...
 {/* Question Text */}
 <div className="mb-6">
 <div className="text-xl font-bold text-text-heading leading-relaxed prose prose-slate dark:prose-invert max-w-none">
 <MarkdownRenderer content={question.question_text} />
 </div>
 {question.question_html && (
 <div
 className="mt-3 text-text-body prose max-w-none"
 ...
 :"border-border-input hover:border-blue-300 dark:hover:border-blue-600 hover:bg-bg-hover"
 }`}
 >
 ...
 <div className="text-text-heading flex-1 prose-sm prose-slate dark:prose-invert max-w-none">
 ...
 className="w-full px-4 py-3 border border-border-input rounded-lg bg-bg-card text-text-heading placeholder:text-text-disabled focus:ring-2 focus:ring-border-focus/20 focus:border-border-focus transition-all"
 ...
 return (
 <div className="min-h-screen bg-bg-root py-4">
 ...
 {/* Header */}
 <div className="bg-bg-card rounded-xl shadow-lg p-6 mb-6">
 <div className="flex justify-between items-start">
 <div>
 <h1 className="text-2xl font-bold text-text-heading">{quiz?.title}</h1>
 <p className="text-text-muted mt-2">{quiz?.description}</p>
 </div>
 {timeLeft !== null && (
 ...
 <div className="w-full bg-bg-section rounded-full h-3">
 <div
 ...
 className="px-6 py-3 bg-accent-primary hover:bg-accent-primary-hover text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
 >
 ← Câu trước
 </Button>
 ...
 :"bg-bg-section text-text-muted border-2 border-border-input"
 }`}
 >
 {idx + 1}
 </button>
