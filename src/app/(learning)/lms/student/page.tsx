...
function EnrolledCourseItem({
 ...
 return (
 <div
 className="flex items-center gap-4 p-4 rounded-xl hover:bg-bg-hover transition-colors cursor-pointer group border border-border-subtle"
 onClick={() => onOpen(enrollment.course_id)}
 >
 {/* Icon */}
 <div className="w-12 h-12 rounded-xl bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-500/20 flex items-center justify-center flex-shrink-0">
 <BookOpen className="w-6 h-6 text-accent-primary dark:text-accent-secondary" />
 </div>
 ...
 Giảng viên: <span className="font-medium text-text-body">{enrollment.teacher_name}</span>
 </p>
 )}
 ...
 {/* Arrow */}
 <ChevronRight className="w-4 h-4 text-text-disabled group-hover:text-accent-primary transition-colors flex-shrink-0" />
 </div>
 );
}
...
"Tổng số": item.total,
 ...
 <div>
 <h1 className="text-3xl font-extrabold text-text-heading leading-tight">
 ...
 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
 {[0, 1, 2].map(i => (
 <div key={i} className="h-24 bg-bg-card rounded-2xl border border-border-subtle animate-pulse" />
 ))}
 </div>
 ) : (
 <LearningStats
 ...
 <div className="px-6 pt-5 pb-4 border-b border-border-subtle flex justify-between items-center bg-bg-section/50 dark:bg-bg-card/50">
 <div>
 ...
 <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between border-b border-border-subtle pb-5 gap-4">
 <div>
 <div className="flex items-center gap-2 text-xs font-semibold text-accent-primary dark:text-accent-secondary uppercase tracking-widest">
 <Target className="w-3.5 h-3.5" />
 Phân tích học tập cá nhân
 </div>
 <h3 className="text-xl font-extrabold text-text-heading mt-1">
 ...
 className="py-2 px-3 border border-border-input rounded-xl text-sm bg-bg-card text-text-heading focus:outline-none focus:ring-2 focus:ring-border-focus/20"
 >
 ...
 <div className="flex items-center gap-1 border-b border-border-subtle pb-3 mb-6 overflow-x-auto">
 <button
 ...
 <div className="bg-bg-root dark:bg-bg-card/30 rounded-2xl border border-border-subtle/50 p-4 flex flex-col h-[320px]">
 <h4 className="font-bold text-text-subheading text-sm mb-3 flex items-center gap-1.5">
 <Layers className="w-4 h-4 text-blue-500" />
 Bài học đã hoàn thành theo loại học liệu
 </h4>
 {!lessonProgress || lessonProgress.total_content === 0 ? (
 <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
 <BookOpen className="w-10 h-10 text-text-disabled dark:text-text-body mb-2" />
 <p className="text-xs text-text-muted">Chưa có bài học nào trong khóa học này.</p>
 </div>
 ) : (
 ...
 <ResponsiveContainer width="100%" height="100%">
  ...
  <Bar dataKey="Chưa học" fill="#cbd5e1" stackId="a" radius={[4, 4, 0, 0]} className="dark:fill-bg-hover" />
  </BarChart>
 </ResponsiveContainer>
 )}
 </div>
 ...
 <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
 <ListTodo className="w-10 h-10 text-text-disabled dark:text-text-body mb-2" />
 <p className="text-xs text-text-muted">Chưa có chương học nào được cấu hình.</p>
 </div>
 ) : (
 <div className="flex-1 overflow-y-auto pr-1 space-y-3">
 {getSectionStats().map((s, idx) => (
 <div key={idx} className="bg-bg-card rounded-xl p-3 border border-border-subtle/50 shadow-xs">
  <div className="flex justify-between items-start gap-3 mb-1.5">
  <span className="font-semibold text-text-subheading text-xs truncate max-w-[70%]">
  {s.section}
  </span>
  <span className="text-xs text-text-muted bg-bg-section px-2 py-0.5 rounded-full font-bold">
 ...
 <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
 <HelpCircle className="w-10 h-10 text-text-disabled dark:text-text-body mb-2" />
 <p className="text-xs text-text-muted">Chưa có đủ dữ liệu tương tác để phân tích độ thông thạo chủ đề.</p>
 </div>
 ) : (
 <div className="flex-1">
 {mounted && (
 ...
 {quizScores.length === 0 ? (
 <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
 <Award className="w-10 h-10 text-text-disabled dark:text-text-body mb-2" />
 <p className="text-xs text-text-muted">Chưa làm bài trắc nghiệm nào trong khóa học này.</p>
 </div>
 ) : (
 ...
 {quizScores.slice(0, 4).map(q => (
  <div key={q.quiz_id} className="bg-bg-card rounded-xl p-2 border border-border-subtle shadow-xs flex items-center justify-between text-xs">
  <span className="font-semibold text-text-body truncate max-w-[80px]">{q.quiz_title}</span>
 ...
 <div className="bg-bg-root dark:bg-bg-card/30 rounded-2xl border border-border-subtle/50 p-4 flex flex-col">
 <h4 className="font-bold text-text-subheading text-sm mb-3 flex items-center gap-1.5">
 <CheckSquare className="w-4 h-4 text-emerald-500" />
 Đánh giá tương tác nhanh (Concept check)
 </h4>
 <div className="grid grid-cols-3 gap-3 text-center text-xs flex-1">
 <div className="bg-bg-card p-3 rounded-xl border border-border-subtle shadow-xs">
 <p className="text-text-muted">Số câu trả lời</p>
 <p className="text-base font-extrabold text-text-subheading mt-1">{microInteractions.total_interactions}</p>
 </div>
 <div className="bg-bg-card/50 p-3 rounded-xl border border-border-subtle shadow-xs">
 <p className="text-text-muted">Số câu trả lời đúng</p>
 <p className="text-base font-extrabold text-emerald-600 dark:text-emerald-400 mt-1">{microInteractions.total_correct}</p>
 </div>
 <div className="bg-bg-card/50 p-3 rounded-xl border border-border-subtle shadow-xs">
 <p className="text-text-muted">Độ chính xác</p>
 <p className="text-base font-extrabold text-accent-primary dark:text-accent-secondary mt-1">
 ...
 <div className="bg-bg-card p-2.5 rounded-xl border border-border-subtle shadow-xs flex justify-between items-center">
 <span className="text-text-muted">Tổng số câu theo dõi:</span>
 <span className="font-bold text-text-subheading">{spacedRepQuizzes.total_tracked}</span>
 </div>
 <div className="bg-bg-card p-2.5 rounded-xl border border-border-subtle shadow-xs flex justify-between items-center">
 <span className="text-text-muted">Cần làm hôm nay:</span>
 <span className={`font-bold ${spacedRepQuizzes.due_today > 0 ?"text-orange-600" :"text-emerald-600"}`}>{spacedRepQuizzes.due_today}</span>
 </div>
 <div className="bg-bg-card p-2.5 rounded-xl border border-border-subtle shadow-xs flex justify-between items-center">
 <span className="text-text-muted">Đã nhớ tốt (Mastered):</span>
 <span className="font-bold text-emerald-600">{spacedRepQuizzes.mastered}</span>
 </div>
 <div className="bg-bg-card p-2.5 rounded-xl border border-border-subtle shadow-xs flex justify-between items-center">
 <span className="text-text-muted">Chất lượng TB:</span>
 ...
 {analyticsTab ==="flashcards" && (
 <div className="bg-bg-root dark:bg-bg-card/30 rounded-2xl border border-border-subtle/50 p-6">
 <h4 className="font-bold text-text-subheading text-sm mb-4 flex items-center gap-1.5">
 <Brain className="w-4 h-4 text-violet-500" />
 Theo dõi ghi nhớ qua Spaced Repetition (Hệ thống thẻ Flashcard)
 </h4>
 {!flashcardStats || flashcardStats.total_active === 0 ? (
 <div className="text-center py-10">
 <Brain className="w-12 h-12 text-text-disabled dark:text-text-body mx-auto mb-2" />
 <p className="text-sm text-text-muted">Chưa có flashcard nào được tạo. Hãy mở bài học và tạo flashcard để bắt đầu ôn tập thông minh.</p>
 </div>
 ) : (
 ...
 <span className="text-2xl font-extrabold text-text-subheading">{flashcardStats.total_active}</span>
 <span className="text-[10px] text-text-muted uppercase font-semibold">Tổng thẻ</span>
 </div>
 </div>

 <div className="space-y-4 flex-1 max-w-sm">
 <div className="grid grid-cols-2 gap-3 text-xs">
 <div className="p-3 bg-bg-card rounded-xl border border-border-subtle shadow-xs">
 <p className="text-text-muted">Cần ôn hôm nay</p>
 ...
 <div className="p-3 bg-bg-card rounded-xl border border-border-subtle shadow-xs">
 <p className="text-text-muted">Đã ôn hôm nay</p>
 <p className="text-lg font-extrabold text-accent-primary dark:text-accent-secondary mt-1">
  {flashcardStats.reviewed_today} thẻ
 </p>
 </div>
 <div className="p-3 bg-bg-card rounded-xl border border-border-subtle shadow-xs">
 <p className="text-text-muted">Độ dễ trung bình (EF)</p>
 <p className="text-lg font-extrabold text-violet-600 dark:text-violet-400 mt-1">
  {flashcardStats.avg_easiness.toFixed(2)}
 </p>
 </div>
 <div className="p-3 bg-bg-card rounded-xl border border-border-subtle shadow-xs">
 <p className="text-text-muted">Tổng số lượt ôn</p>
 <p className="text-lg font-extrabold text-text-body mt-1">
 ...
 <Card className="p-8 text-center bg-bg-root dark:bg-bg-card/20 border border-dashed border-border-input dark:border-border-subtle rounded-2xl flex flex-col items-center justify-center min-h-[300px]">
 <Target className="w-12 h-12 text-text-disabled dark:text-text-body mb-3" />
 <h3 className="text-lg font-bold text-text-subheading">Không tìm thấy phân tích</h3>
 <p className="text-sm text-text-muted mt-1">Đăng ký và tham gia một khóa học để bắt đầu ghi nhận phân tích học tập.</p>
 </Card>
 )}
 </div>
