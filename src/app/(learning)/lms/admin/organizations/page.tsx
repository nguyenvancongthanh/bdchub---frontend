...
const ROLE_COLORS: Record<string, string> = {
 ...
 MEMBER:"bg-bg-section text-text-body",
};
...
function OrgCard({ org, onSelect, onDeactivate }: OrgCardProps) {
 ...
 return (
 <div
 className={`relative bg-bg-card rounded-2xl border shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden
 ${org.is_active ?"border-border-subtle" :"border-red-200 dark:border-red-900/40 opacity-70"}`}
 >
 {/* Header stripe */}
 ...
 <Building2 className="w-5 h-5 text-accent-primary dark:text-accent-secondary" />
 </div>
 <div className="min-w-0">
 <h3 className="font-bold text-text-heading text-sm leading-tight truncate">
 {org.name}
 </h3>
 ...
 className="p-1.5 rounded-lg hover:bg-bg-hover transition-colors active:scale-95"
 >
 <MoreVertical className="w-4 h-4 text-text-muted" />
 </button>
 {menuOpen && (
 <div className="absolute right-0 top-full mt-1 z-20 w-40 bg-bg-card border border-border-input rounded-xl shadow-lg py-1">
 <button
 onClick={() => { onSelect(org); setMenuOpen(false); }}
 className="w-full flex items-center gap-2 px-3 py-2 text-sm text-text-body hover:bg-bg-hover transition-colors"
 >
 ...
 <span className="flex items-center gap-1 px-2.5 py-1 bg-bg-section border border-border-input rounded-full text-xs text-text-muted">
 ...
 className="w-full flex items-center justify-center gap-2 py-2 rounded-xl bg-bg-section border border-border-input text-sm font-medium text-text-body hover:bg-blue-50 dark:hover:bg-blue-950/30 hover:border-border-hover hover:text-blue-700 dark:hover:text-blue-400 transition-all duration-200 active:scale-95"
 >
 <Settings className="w-3.5 h-3.5" /> Manage
...
"MEMBER"
 ...
 {loading ? (
 <div className="h-6 w-12 bg-bg-section rounded animate-pulse" />
 ) : (
 ...
 <button
 onClick={onBack}
 className="p-2 rounded-xl hover:bg-bg-hover transition-colors active:scale-95"
 >
 ...
 {/* Members section */}
 <div className="bg-bg-card rounded-2xl border border-border-subtle overflow-hidden shadow-sm">
 {/* Header */}
 ...
 <span className="px-1.5 py-0.5 bg-bg-section rounded-full text-xs text-text-muted">
 {totalMembers}
 </span>
 </div>
 <button
 ...
 className="flex items-center gap-1.5 px-3 py-1.5 bg-accent-primary hover:bg-accent-primary-hover text-white rounded-xl text-xs font-semibold transition-all duration-200 active:scale-95 shadow-sm"
 >
 ...
 {/* Add member form */}
 {showAddMember && (
 <div className="px-5 py-4 bg-bg-section dark:bg-bg-hover border-b border-border-subtle">
 {error && (
 ...
 <p>• Added: <span className="font-semibold text-text-subheading">{(bulkResult.added || []).length}</span> members.</p>
 ...
 * Note: These users must be created or synced in the LMS database first.
 ...
 {/* Tab navigation */}
 <div className="flex border-b border-border-input mb-4 gap-4">
 <button
 type="button"
 ...
 addMode ==="single"
 ?"border-blue-500 text-accent-primary dark:text-accent-secondary"
 :"border-transparent text-text-muted hover:text-text-body dark:hover:text-text-disabled"
 }`}
 >
 Single Member
 </button>
 ...
 className="flex-1 px-4 py-2.5 rounded-xl border border-border-input bg-bg-card text-sm text-text-heading placeholder:text-text-disabled focus:border-border-focus focus:ring-2 focus:ring-border-focus/20 outline-none transition-all duration-200"
 />
 <select
 id="add-member-role"
 value={addRole}
 onChange={(e) => setAddRole(e.target.value as"OWNER" |"ADMIN" |"MEMBER")}
 className="px-4 py-2.5 rounded-xl border border-border-input bg-bg-card text-sm text-text-heading focus:border-border-focus focus:ring-2 focus:ring-border-focus/20 outline-none transition-all duration-200"
 >
 ...
 className="px-5 py-2.5 bg-accent-primary hover:bg-accent-primary-hover disabled:opacity-50 text-white rounded-xl text-sm font-semibold transition-all duration-200 active:scale-95 shadow-sm"
 >
 ...
 className="w-full px-4 py-3 rounded-xl border border-border-input bg-bg-card text-sm text-text-heading placeholder:text-text-disabled focus:border-border-focus focus:ring-2 focus:ring-border-focus/20 outline-none transition-all duration-200 resize-none"
 />
 
 {parsedEmails.length > 0 && (
 <div className="p-3 bg-bg-section rounded-xl">
 <p className="text-xs font-semibold text-text-body mb-2">
 Detected Emails ({parsedEmails.length}):
 </p>
 <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto">
 {parsedEmails.map((email) => (
 <span key={email} className="px-2 py-0.5 bg-blue-50 dark:bg-blue-900/40 text-accent-primary dark:text-accent-secondary border border-blue-100 dark:border-blue-800 rounded-lg text-[10px] font-medium">
 {email}
 </span>
 ))}
 </div>
 ...
 value={addRole}
 onChange={(e) => setAddRole(e.target.value as"OWNER" |"ADMIN" |"MEMBER")}
 className="px-4 py-2 rounded-xl border border-border-input bg-bg-card text-sm text-text-heading focus:border-border-focus focus:ring-2 focus:ring-border-focus/20 outline-none transition-all duration-200"
 >
 <option value="MEMBER">Member</option>
 <option value="ADMIN">Admin</option>
 <option value="OWNER">Owner</option>
 ...
 className="px-5 py-2 bg-accent-primary hover:bg-accent-primary-hover disabled:opacity-50 text-white rounded-xl text-sm font-semibold transition-all duration-200 active:scale-95 shadow-sm"
 >
 ...
 <div
 key={i}
 className="h-12 bg-bg-section rounded-xl animate-pulse"
 />
 ))}
 </div>
 ) : members.length === 0 ? (
 <div className="py-12 text-center">
 <Users className="w-8 h-8 text-text-disabled dark:text-text-muted mx-auto mb-2" />
 ...
 className="px-3 py-1.5 rounded-xl text-xs border border-border-input disabled:opacity-40 hover:bg-bg-hover transition-colors active:scale-95"
 >
 Prev
 </button>
 <button
...
function CreateOrgModal({ open, onClose, onCreated }: CreateOrgModalProps) {
 ...
 onClick={onClose}
 />
 <div className="relative z-10 w-full max-w-md bg-bg-card rounded-2xl border border-border-subtle shadow-2xl">
 {/* Header */}
 ...
 <Building2 className="w-4.5 h-4.5 text-accent-primary dark:text-accent-secondary" />
 </div>
 ...
 className="w-full px-4 py-2.5 rounded-xl border border-border-input bg-bg-section text-sm text-text-heading placeholder:text-text-disabled focus:border-border-focus focus:ring-2 focus:ring-border-focus/20 outline-none transition-all duration-200"
 />
 </div>
 ...
 <label className="block text-xs font-semibold text-text-body mb-1.5">
 Slug *{""}
 <span className="font-normal text-text-disabled">(URL identifier)</span>
 </label>
 <div className="flex items-center gap-2">
 ...
 placeholder="bdc"
 className="flex-1 px-4 py-2.5 rounded-xl border border-border-input bg-bg-section text-sm font-mono text-text-heading placeholder:text-text-disabled focus:border-border-focus focus:ring-2 focus:ring-border-focus/20 outline-none transition-all duration-200"
 />
 </div>
 </div>
 ...
 className="w-full px-4 py-2.5 rounded-xl border border-border-input bg-bg-section text-sm text-text-heading placeholder:text-text-disabled focus:border-border-focus focus:ring-2 focus:ring-border-focus/20 outline-none transition-all duration-200 resize-none"
 />
 </div>
 ...
 type="button"
 onClick={onClose}
 className="flex-1 py-2.5 rounded-xl border border-border-input text-sm font-medium text-text-muted hover:bg-bg-hover transition-colors active:scale-95"
 >
 Cancel
 </button>
 <button
 ...
 className="flex-1 py-2.5 rounded-xl bg-accent-primary hover:bg-accent-primary-hover disabled:opacity-50 text-white text-sm font-semibold shadow-sm transition-all duration-200 active:scale-95"
 >
...
export default function OrganizationsPage() {
 ...
 className="pl-8 pr-4 py-2 rounded-xl border border-border-input bg-bg-card text-sm text-text-heading placeholder:text-text-disabled focus:border-border-focus focus:ring-2 focus:ring-border-focus/20 outline-none transition-all duration-200 w-48"
 />
 </div>
 ...
 className="flex items-center gap-2 px-4 py-2 bg-accent-primary hover:bg-accent-primary-hover text-white rounded-xl text-sm font-semibold shadow-sm transition-all duration-200 active:scale-95"
 >
 ...
 <div
 key={i}
 className="h-48 bg-bg-section rounded-2xl animate-pulse"
 />
 ))}
 </div>
 ) : orgs.length === 0 ? (
 <div className="py-20 text-center">
 <Building2 className="w-12 h-12 text-text-disabled dark:text-text-body mx-auto mb-3" />
 <p className="text-text-muted font-medium">No organizations found</p>
 {search && (
 ...
 className="px-4 py-2 rounded-xl border border-border-input text-sm text-text-muted disabled:opacity-40 hover:bg-bg-hover transition-colors active:scale-95"
 >
 Previous
 </button>
